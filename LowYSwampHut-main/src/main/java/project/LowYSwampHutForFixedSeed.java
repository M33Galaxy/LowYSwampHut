...
public class LowYSwampHutForFixedSeed extends JFrame {
    ...
    // 注意：结果里不再存在"无法生成"标记（老的 × / "x" 后缀随 SeedChecker 回退路径一起删除，
    // 现在拒绝在精确群系门处提前发生），所以每一行都是有效结果，直接按 y 排序。
    ...
    private void updateSearchProgress(SearchCoords.ProgressInfo info) {
        SwingUtilities.invokeLater(() -> {
            ...
            // 单一进度的完成条件：已扫完的 region 数达到总 region 数。
            // 没有阶段，所以这里只看 processed >= total —— 这也是 SearchCoords 里
            // 唯一会发出 processed == total == 总 region 数 的那次上报。
            if (info.total() > 0 && info.processed() >= info.total()) {
                isSearchRunning = false;
                isSearchPaused = false;
                searchStartButton.setEnabled(true);
                searchPauseButton.setEnabled(false);
                            ...
                            if (!seedParallel) {
                                seedProgressCallback = info -> {
                                    ...
                                    SwingUtilities.invokeLater(() -> {
                                        if (isListSearchRunning) {
                                            listSearchCurrentSeedProgressLabel.setText(
                                                    getString("currentSeed", currentSeedIndex, totalSeeds,
    ...
    // 按最低y排序（结果里已不存在"无法生成"标记，每个种子都取最低 y）
    private void sortListByLowestY() {
        ...
        // 按最低y值排序（从低到高）
        seedYList.sort(Comparator.comparingDouble(Map.Entry::getValue));
    ...
    // 按距离原点由近到远排序（结果里已不存在"无法生成"标记，每个种子都取最近距离）
    private void sortListByDistance() {
        ...
        // 按距离排序（从近到远，即从小到大）
        seedDistanceList.sort(Comparator.comparingDouble(Map.Entry::getValue));
    ...
    public static void main(String[] args) {
        // 计算全部在 C 内核（lysh.dll）里；SeedChecker 初始化已随 Java 老路径一起删除。
        // 缺库时明确告知用户，而不是让他点"开始搜索"后一无所获。
        boolean nativeReady = NativePhase1.isAvailable() && NativePhase2.isActive();
        if (!nativeReady) {
            System.err.println("[engine] " + SearchCoords.nativePathSummary());
        }
        SwingUtilities.invokeLater(() -> {
            try {
            ...
            if (!nativeReady) {
                JOptionPane.showMessageDialog(null,
                        "lysh.dll could not be loaded, so no search can run.\n"
                                + "Put lysh.dll in the same folder as the jar, or pass\n"
                                + "-Dlowyswamphut.nativeLib=<path to lysh.dll>.\n\n"
                                + "未找到或无法加载 lysh.dll，无法进行搜索。\n"
                                + "请把 lysh.dll 与 jar 放在同一目录。",
                        "LowYSwampHut", JOptionPane.ERROR_MESSAGE);
            }
        });
    }
}
